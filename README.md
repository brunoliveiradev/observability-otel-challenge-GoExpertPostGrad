...
Você pode acessar o **serviço A** em `http://localhost:8080/` e enviar um cep valido no formato JSON. O
...
Você pode acessar o serviço B em `http://localhost:8081/{cep}`. O arquivo `api/get_temperatures.http` contém exemplos de
uso.
...
You can access service A at `http://localhost:8080/` and send a valid zip code in JSON format.
The `api/get_temperatures.http` file contains usage examples.
...
After starting the development environment, you can test with the example cURL below or with
the `api/get_temperatures.http` file:
